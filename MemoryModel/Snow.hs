  ...
  connectPrograms mem cond from to ptrs = do
    ...
    let (_,env_from) = (snowLocs mem)!from
        ...
        new_env1 = foldl (\mp (ptr_to,ptr_from)
                         ...
                         ) (Map.union env_to (fmap (\(obj_p,tp) -> decision (obj_p,[(tp,[])]))
...
updateLocation :: (Ord ptr,Show ptr) => Map String [TypeDesc] 
                  -> SMTExpr Bool 
                  -> Map ptr (DecisionTree (Integer,PtrIndex))
                  -> Map Integer (Object ptr)
                  -> Integer
                  -> [MemoryInstruction ptr] 
                  -> SMT (Map ptr (DecisionTree (Integer,PtrIndex)),
               ...
               MIStore val ptr -> case Map.lookup ptr ptrs of
                 Just dt -> do
                   let (nnext,nobjs,ups)
                       ...
                       nptrs = fmap (fmap (\(obj_p,idx) -> case Map.lookup obj_p ups of
                                              Nothing -> (obj_p,idx)
